...
async function mintpublic(how_many_tokens) {
    if (xCnyContract) {
      ...
      const gasAmount = await xCnyContract.methods.publicSaleMint(1).estimateGas({from: walletAddress, value: price})
...
<span className="flex PalatinoLinotype text-3xl gold-color items-center bg-grey-lighter rounded rounded-r-none my-4 ">Price: WL = 0.066 / Public = 0.088</span>
                ...
                {saleStarted ? 
                <button onClick={() => mintpublic(how_many_tokens)} className="mt-4 PalatinoLinotype text-2xl rounded-full border-6 bg-white text-black py-4 px-6 hover:text-red-900 ">MINT</button>        
